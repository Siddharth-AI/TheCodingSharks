...
import { ArrowRight, Download, Sparkles, Code2, Terminal, Blocks } from "lucide-react";
...
export function HeroSection() {
  ...
  return (
    <Section className="relative overflow-hidden min-h-[calc(100svh-4rem)] sm:min-h-[calc(100svh-5rem)] md:min-h-[calc(100svh-6rem)] flex flex-col justify-center bg-transparent">
      <style>{styles}</style>
      
      {/* Background Elements */}
      <div className="pointer-events-none absolute inset-0 -z-10 bg-black overflow-hidden">
        {/* Subtle top glow */}
        ...
        {/* Global Grid Overlay with Subtle Primary Tint */}
      ...
      <div className="absolute hidden lg:flex top-[8%] left-[10%] float-icon text-white/5 pointer-events-none">
        <Code2 size={64} strokeWidth={1} className="text-primary/50" />
      </div>
      <div className="absolute hidden lg:flex top-[50%] right-[8%] float-icon text-white/5 pointer-events-none" style={{ animationDelay: '1.5s' }}>
        <Terminal size={56} strokeWidth={1} className="text-primary/50" />
      </div>
      <div className="absolute hidden sm:flex bottom-[5%] left-[20%] float-icon text-white/5 pointer-events-none" style={{ animationDelay: '3s' }}>
        <Blocks size={72} strokeWidth={1} className="text-primary/30" />
      </div>
      ...
      <Container className="relative z-10 w-full">
        <div className="flex flex-col items-center text-center px-2 sm:px-0">
          ...
          {/* Eyebrow badge */}
          <div className="animate-fade-slide-1 mb-6 sm:mb-8 md:mb-10 inline-flex items-center gap-2 rounded-full border border-orange-500/20 bg-[#211832]/40 px-3 sm:px-4 py-1 sm:py-1.5 backdrop-blur-md">
            <span className="text-[10px] sm:text-xs font-bold tracking-[0.15em] sm:tracking-[0.2em] text-white/80 uppercase">
              {hero.eyebrow}
            </span>
          </div>
          ...
          <h1 className="animate-fade-slide-2 max-w-sm sm:max-w-3xl md:max-w-4xl lg:max-w-6xl xl:max-w-7xl text-balance font-medium tracking-tight font-heading">
            {/* Line 1 — "Build Real Software Products" */}
            <span className="text-white block sm:whitespace-nowrap text-4xl sm:text-5xl md:text-6xl lg:text-[72px] xl:text-[80px] leading-[1.1]">
              {hero.headline1}{" "}
              <span className="inline-block relative bg-orange-400/10 border border-orange-400/20 p-1 sm:p-1.5 md:p-2">
                <span>{hero.headline2}</span>
                <span className="absolute -top-1.5 -right-1.5 sm:-top-2 sm:-right-1.5 h-2 w-2 sm:h-3 sm:w-3 rounded-full bg-primary/90 animate-pulse"></span>
                <span className="absolute -top-1.5 -left-1.5 sm:-top-2 sm:-left-1.5 h-2 w-2 sm:h-3 sm:w-3 rounded-full bg-primary/90 animate-pulse"></span>
                <span className="absolute -bottom-1.5 -right-1.5 sm:-bottom-2 sm:-right-1.5 h-2 w-2 sm:h-3 sm:w-3 rounded-full bg-primary/90 animate-pulse"></span>
                <span className="absolute -bottom-1.5 -left-1.5 sm:-bottom-2 sm:-left-1.5 h-2 w-2 sm:h-3 sm:w-3 rounded-full bg-primary/90 animate-pulse"></span>
              </span>
            </span>

            {/* Line 2 — italic accent phrase */}
            <span className="inline-flex items-center justify-center gap-2 sm:gap-4 text-primary italic font-heading opacity-90 relative mt-6 sm:mt-8 md:mt-8">
              <span className="hidden sm:inline-block w-8 sm:w-12 h-[2px] bg-primary/50 relative -bottom-4 sm:-bottom-5"></span>
              <span className="relative z-10 inline-block text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-[80px]font-normal leading-[1]">
                {hero.headline3.split('—')[1]?.trim() || "Not Just Learn Coding"}
                {/* Underline SVG */}
                <span className="pointer-events-none absolute -bottom-4 sm:-bottom-5 md:-bottom-6 left-1/2 -translate-x-1/2 w-[110%] sm:w-[120%] text-primary opacity-90 drop-shadow-lg">
          ...
          {/* Subheadline */}
          <div className="animate-fade-slide-3 mt-12 sm:mt-16 md:mt-18 max-w-sm sm:max-w-xl md:max-w-2xl lg:max-w-3xl">
          ...
          <div className="animate-fade-slide-5 mt-8 sm:mt-10 md:mt-16 flex flex-col gap-3 sm:gap-4 sm:flex-row sm:items-center sm:justify-center w-full sm:w-auto">
            <ButtonLink
              ...
              size="lg"
              className="group h-11 sm:h-12 md:h-14 w-full sm:w-auto rounded-full bg-primary px-6 sm:px-8 text-sm sm:text-base font-semibold text-primary-foreground shadow-[0_4px_14px_0_rgba(255,107,44,0.39)] hover:shadow-[0_6px_20px_rgba(255,107,44,0.23)] hover:bg-primary/90 transition-all flex items-center justify-center gap-2">
            ...
            {secondaryCta && (
              <ButtonLink
                ...
                size="lg"
                className="group h-11 sm:h-12 md:h-14 w-full sm:w-auto rounded-full border border-white/20 bg-transparent hover:bg-white/5 px-6 sm:px-8 text-sm sm:text-base font-medium text-white transition-all flex items-center justify-center gap-2">
