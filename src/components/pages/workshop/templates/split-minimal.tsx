...
"--cta-ring":`${P}22`,
 ...
 />
   ...
   <>
     {(media?.hero_preview || media?.hero_bg) && (
       <Image src={media?.hero_preview ?? media?.hero_bg ?? ""} alt={workshop.title} fill className="object-cover" sizes="(max-width:768px) 100vw, 660px" priority />
     )}
     <div className="absolute inset-0 bg-black/30 flex items-center justify-center cursor-pointer" onClick={() => setShowHeroVideo(true)}>
       <div className="w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center text-white shadow-2xl" style={{ backgroundColor: P }}>
         <svg className="w-7 h-7 sm:w-9 sm:h-9 ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
       </div>
     </div>
   </>
 )
 ...
 />
 <div
 className="w-8 h-8 flex items-center justify-center shrink-0 my-2"
 style={{ backgroundColor: P }}>
 <svg
 ...
 {/* Dot */}
 <div
 className="relative z-10 w-8 h-8 border-2 flex items-center justify-center shrink-0 bg-white text-xs font-black"
